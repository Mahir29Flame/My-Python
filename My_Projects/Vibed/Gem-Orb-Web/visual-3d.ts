...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private init() {
    ...
    new EXRLoader().load(
      ...
      (error) => {
        ...
        // Fallback: make sphere visible anyway with default material settings
        // so it's not just a black screen if the texture fails.
        sphere.visible = true;
      },
    );
  ...
  protected firstUpdated() {
    // Fix: Cast `this` to `HTMLElement` to make `shadowRoot` accessible to the TypeScript compiler.
    // FIX: Cast `this` to `unknown` before casting to `HTMLElement` to fix TypeScript error.
    this.canvas = (this as unknown as HTMLElement).shadowRoot!.querySelector('canvas') as HTMLCanvasElement;
    this.init();
  }
